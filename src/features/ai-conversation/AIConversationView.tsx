...
export function AIConversationView() {
  ...
  <div className={`ai-teacher ai-teacher--${stage}`}>
    ...
    {stage === "review" && practice ? <section className="ai-flow"><div className="ai-review-layout"><div className="ai-review-card">
      ...
      <fieldset className="op-time-range"><legend>Response time (minutes)</legend><strong>{responseTime(practice)}</strong><div className="op-range-labels"><span>Minimum: {practice.minResponseTime ?? 1} min</span><span>Maximum: {practice.maxResponseTime ?? 2} min</span></div><div className="op-dual-range"><input aria-label="Minimum response time" type="range" min="1" max="5" value={practice.minResponseTime ?? 1} onChange={(event) => { const min = Number(event.target.value); setPractice({ ...practice, minResponseTime: min, maxResponseTime: Math.max(min, practice.maxResponseTime ?? 2) }); }} /><input aria-label="Maximum response time" type="range" min="1" max="5" value={practice.maxResponseTime ?? 2} onChange={(event) => { const max = Number(event.target.value); setPractice({ ...practice, maxResponseTime: max, minResponseTime: Math.min(max, practice.minResponseTime ?? 1) }); }} /></div><small>Choose between 1 and 5 minutes.</small></fieldset>
